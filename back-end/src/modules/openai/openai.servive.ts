  ...
  async updateApiKeyOpenAI(
    ...
    console.log(openai);
      ...
      try {
        ...
        if (myAssistants.data.filter(assistant => assistant.name === 'Assistant for Q&A system').length === 0){
        ...
        assistant = myAssistants.data.find(assistant => assistant.name === 'Assistant for Q&A system')[0];
      }
      } catch (error) {
...
async generateQuestions(file: Express.Multer.File, examId: number): Promise<any> {
  try {
