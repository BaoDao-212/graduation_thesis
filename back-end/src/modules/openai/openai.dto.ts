...
export const prompt1 = ` Create a set of about `;
export const prompt11 = `questions with answers in json format in English and add it to the file. let me download it. The questions will range from easy(0), medium(1), difficult(2) and very difficult(3). Difficult and very difficult level questions require more in-depth knowledge
...
With output of the questions in json format:
  ...
  Note: Analysis and evaluation must include detailed analysis of correct and incorrect answers, along with suggestions for improvement and related resources, but must be concise and not lengthy.If the number of correct answers is less than 4 and the number of incorrect answers is greater than 10, there will be no content in the correctly_answered_questions section.
`;
